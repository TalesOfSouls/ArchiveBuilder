...
void enum_name_from_path(const char* path, char* enum_name) {
    ...
    const char* last_dir = strrchr(path, '/');
    if (!last_dir) {
    ...
    strcpy(enum_name, file_name);

    // Remove file extension
    char* dot = strrchr(enum_name, '.');
    if (dot != NULL) {
        *dot = '_';
    }

    // If the file contains multiple . as file extension we ignore the last one
    // The reason for this is that we sometimes need platform/api specific file names
    // but in our enum we need the same name between all enums (e.g. compiled vulkan shaders)
    char* dot2 = strrchr(enum_name, '.');
    ...
    for (int32 c = 0; enum_name[c] != '\0'; ++c) {
        if (isalnum(enum_name[c])) {
            enum_name[c] = (char) toupper(file_name[c]);
        } else {
            enum_name[c] = '_';
        }
    }
...
void build_asset(
    ...
    byte** archive_body, const byte* body_start,
    char* enum_char,
    AssetArchiveHeader* header,
    int32 archive_id
) {
    char abs_input_path[PATH_MAX_LENGTH] = {0};
    if (*input_path == '.') {
        memcpy(abs_input_path, rel_path, PATH_MAX_LENGTH);
        strcpy(abs_input_path + strlen(abs_input_path), input_path + 1);
    } else {
        strcpy(abs_input_path, input_path);
    }
    ...
    enum_name_from_path(abs_input_path, &enum_char[64 * header->asset_count]);

    // Get the file extension of the asset (we handle file types differently)
    const char* extension = strrchr(abs_input_path, '.');
    ...
    element->start = (uint32) ((uintptr_t) *archive_body - (uintptr_t) body_start);
    ...
    if (strncmp(extension, ".wav", sizeof("wav") - 1) == 0) {
        ...
        *archive_body += element->uncompressed;
        ...
        atlas_from_file_txt(&atlas, abs_input_path, memory_volatile);
        ...
        *archive_body += element->uncompressed;
        ...
        // @question Consider to pull out
        bool is_external = str_is_num(atlas.texture_name);
        int32 texture_id;

        int32 dependency_index = element->dependency_count++;
        ...
        if (is_external) {
            ...
            if (atlas.texture_name[0] != '.') {
                memcpy(dependency_input_path, "./", sizeof("./"));
                i = 2;
            }

            for (const char* pos = atlas.texture_name; *pos; ++pos) {
                dependency_input_path[i++] = (char) *pos;
            }
        ...
        *archive_body += element->uncompressed;
        ...
        if (strncmp(extension, ".png", sizeof("png") - 1) == 0) {
            ...
            int32 w2,h2;
            byte* data2 = stbi_load(abs_input_path, &w2, &h2, 0, 4);
...
)
{
    ...
    const int32 header_offset = (int32) (sizeof(header.version)
        + sizeof(header.asset_count)
        + sizeof(header.asset_dependency_count)
...
int32 main(int32 argc, char* argv[])
{
    ...
    char* enum_char = (char *) malloc(sizeof(char) * 64 * 1024);
